...
train_parser.add_argument('-seendl', '--seen_domain_list', type=str, nargs='+', default=seendl,
                    help='domain list -- example: jp cn')
train_parser.add_argument('-traindl', '--train_domain_list', type=str, nargs='+', default=seendl,
                    help='domain list -- example: jp cn')
train_parser.add_argument('-testdl', '--test_domain_list', type=str, nargs='+', default=testdl,
                    help='domain list -- example: jp cn')
train_parser.add_argument('-s2t', '--seen2test', type=str, default=seen2test,
...
train_parser.add_argument('-sn', '--self_neg', type=int, default=1,
                         help='0: no domain balance; 1: domain balance')
train_parser.add_argument('-ls', '--lr_schedule', type=int, default=1,
                         help='0: no lr_schedule')

# model section
train_parser.add_argument('-F', '--nlp_feature', type=int, default=768,
                         help='nlp feature')
train_parser.add_argument('-H', '--H', type=int, default=256,
                         help='rec feature')
...
train_parser.add_argument('-pt', '--pop_type', type=str, default='pop1',
                          help= 'pop_type')
train_parser.add_argument('-pd', '--pop_d', type=int, default=4,
...
# training section
train_parser.add_argument('-lr', '--lr', type=float, default=0.0003,
                         help='learning rate')
...
train_parser.add_argument('-model_flag', '--model_flag', type=int, default=999,
                          help= 'model_flag')
...
from visual_user_utils import train_valid, test
...
if 1:
    ...
    model.load_state_dict(torch.load(save_folder+'/our5.pt'))
    ...
    for ep in np.arange(1):
        ...
        with torch.no_grad():
            ...
            test_dict, I_userRec_c1 = test(seen_asin2_,
                             seen_user2_,
            ...
            print('confounder = ', test_confounder)
            ...
            with open('I_userRec_c1.pickle', 'wb') as handle:
                pickle.dump(I_userRec_c1, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
            
            test_dict, I_userRec_c0 = test(seen_asin2_,
                             seen_user2_,
                             model, 0, test_item_debias, test_use_pop,
                             train_domain_list,
                             split_test_dataloader,
                             train_I2B,
                             method=method,
            ...
            with open('I_userRec_c0.pickle', 'wb') as handle:
                pickle.dump(I_userRec_c0, handle, protocol=pickle.HIGHEST_PROTOCOL)
